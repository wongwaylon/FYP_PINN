...
class PINN(nn.Module):
    def __init__(
        ...
        lays=10, # number of hidden layers
    ...
    def load_data(self, batch, t0_data, r0_data, r1_data, phy_data, val_data, I):
    ...
    def change_data(self):
        # N = self.phy_N
        new_t = torch.rand(self.phy_N,1)
        # print(new_t)
        new_r = torch.rand(self.phy_N,1)
        new_c = torch.zeros(self.phy_N,1)
    ...
    def pinn_loss(self):
        ...
        bc1 = self.rmax_cbounds(r_r1, t_r1)
    ...
    def train_adam(self, epochs=10000, lr=1e-3, plots=1, update=1000, start=5000, data_change=False):
        # assign the learning rate to ADAM
        ...
        for epoch in range(epochs):
            ...
            with torch.autograd.no_grad():
                print(epoch,"Traning Loss:",loss.data, self.t0_loss.data, self.r0_loss.data, self.r1_loss.data, self.pde_loss.data)
                # print(epoch,"Traning Loss:",loss.data)
                losses.append(loss.data)
                t0_losses.append(self.t0_loss.data)
                r0_losses.append(self.r0_loss.data)
                r1_losses.append(self.r1_loss.data)
                phy_losses.append(self.pde_loss.data)
                
            val_losses = self.val_loss()
            rms_losses.append(val_losses.data)
            
            if data_change == True:
                if epoch % update == 0 and epoch >= start:
                    
                    r=np.linspace(0,1,101) # 100 points
                    t=np.linspace(0,1,101)
                    net_r, net_t, ms_r, ms_t = meshgrid_plot(r,t)
                    c_pde = net.phy_loss(net_r, net_t)
            ...
            if epoch % (epochs/plots) == 0:
                self.visualisation(epoch)
                
        adam_losses = [losses,t0_losses,r0_losses,r1_losses,phy_losses,rms_losses]
        return adam_losses
    ...
    def train_lbfgs(self, epochs=1000):
        lbfgs_loss = []
        lbfgs_t0 = []
        lbfgs_r0 = []
        lbfgs_r1 = []
        ...
        for epoch in range(epochs):
            ...
            with torch.autograd.no_grad():
                ...
                print(epoch,"LBFGS Loss:",self.train_loss.data, self.t0_loss.data, self.r0_loss.data, self.r1_loss.data, self.pde_loss.data)
        ...
        losses = [lbfgs_loss,lbfgs_t0,lbfgs_r0,lbfgs_r1,lbfgs_pde,lbfgs_val]        
        return losses
    
    def train_loop(self, adam_epochs=10000, adam_lr=1e-3, lbfgs_epochs=100, runs=1, data_change=False, plots=1):
        adam_loss = []
        adam_t0 = []
        adam_r0 = []
        adam_r1 = []
...
# data loading
N = 1024 # training data
batch = 256 # batchsize

zero_vals = torch.zeros(N,1)
one_vals = torch.ones(N,1)
c_ini_vals = torch.full((N,1),c_ini_norm).float()
I_vals = torch.full((N,1),I).float()
# rand_vals = torch.rand(N,1)
grid = np.linspace(0,1,32)
phy_r, phy_t, r_grid, t_grid = meshgrid_plot(grid,grid)
phy_r = phy_r.detach().cpu()
phy_t = phy_t.detach().cpu()
phy_r = torch.rand(N,1)
phy_t = torch.rand(N,1)
# t, r, c order
t0_data = TensorDataset(zero_vals, torch.rand(N,1), c_ini_vals)
r0_data = TensorDataset(torch.rand(N,1), zero_vals, zero_vals)
r1_data = TensorDataset(torch.rand(N,1), one_vals, I_vals)
# phy_data = TensorDataset(torch.rand(N,1), torch.rand(N,1), zero_vals) # for rand
phy_data = TensorDataset(phy_t, phy_r, torch.zeros(len(phy_r),1)) # for grid
# t0_t, t0_r, t0_c, r0_t, r0_r, r0_c, r1_t, r1_r, r1_c, phy_t, phy_r, phy_c
# data = TensorDataset(zero_vals, rand_vals, c_ini_vals, rand_vals, zero_vals, zero_vals, rand_vals, one_vals, I_vals, rand_vals, rand_vals, zero_vals)
...
val_data = TensorDataset(val_r, val_t, val_c)

# print(len(val_data))

net = PINN(input_dim, output_dim, lays=layers, n_units=per_layer, loss=mse_loss_func,device=device).to(device)
# print(net)

net.vals(tmax=tmax,rmax=Rs,cmax=c_max,D=D,theta_norm=theta_norm)
net.load_data(batch,t0_data,r0_data,r1_data,phy_data,val_data, I)


# %% run sim

plt.scatter(phy_t.detach(), phy_r.detach())
plt.title("PDE new data")
plt.xlabel("t")
plt.ylabel("r")
plt.xlim(-0.05, 1.05)
...
# adam_losses = net.train_adam(epochs=adam_epochs, lr=lr, plots=5, data_change=change, update=adam_update, start=adam_start)
# visualise losses v epochs

# lbfgs_losses = net.train_lbfgs(epochs=lbfgs_epochs)

#%%
adam_losses, lbfgs_losses = net.train_loop(adam_epochs=adam_epochs, adam_lr=lr, lbfgs_epochs=lbfgs_epochs, plots=5)

#%%
# adaptive_epochs = 1000
# runs = 1
...
plt.figure(1)
plt.plot(Rs*ms_r[...,500],c_max*ms_c[...,500], label = "neural_network")
plt.plot(Rs*CVM_r,c_max*CVM_c[...,500], label = 'CVM_soln')
plt.legend()
plt.xlabel("r")
...
# vid.save('moving_t.gif',writer='ffmpeg',fps=60)
# plt.show
...
# writervideo = ani.FFMpegWriter(fps=60)
# vid.save('moving_r.gif',writer='ffmpeg',fps=60)
# plt.show