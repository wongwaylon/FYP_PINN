# Solve solid spherical diffusion with stress impact based on [Zhang et al., JES, 2007]
# Author: HY
...
times = 2000
c_guess = c_prev
c_all = np.zeros((N,times+1))
for hh in range(times):
...
# c > 1 at Rs when t=1663s for nonzero theta
# c > 1 at Rs when t=1606s for theta=0
...
plt.xlim(0, 6e-6)
plt.ylabel('Concentration (mol/m3)')
plt.title(f'Concentration at t={times}')
plt.show()
...
save_t = pd.DataFrame(np.arange(0,2001,1))
